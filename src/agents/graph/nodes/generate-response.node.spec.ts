...
describe('generateResponseNode', () => {
  ...
  it('should generate response for collect_birth_date step', async () => {
    ...
    expect(result.response).toBe('Prazer, Maria! Qual é a sua data de nascimento?');
    const callArgs = (mockLLM.invoke as jest.Mock).mock.calls[0][0];
  ...
  it('should generate response for collect_weight_loss_reason step', async () => {
    ...
    const mockResponse = 'Qual é o principal motivo para você querer emagrecer?';
    jest.spyOn(mockLLM, 'invoke').mockResolvedValue(mockResponse);

    const result = await generateResponseNode(state);

    expect(result.response).toBe('Qual é o principal motivo para você querer emagrecer?');
    const callArgs = (mockLLM.invoke as jest.Mock).mock.calls[0][0];
    expect(callArgs).toContain('Maria');
  ...
  it('should generate response for qualified step', async () => {
    const state: ConversationState = {
      phoneNumber: '5511999999999',
      messages: [
        { role: 'user', content: 'Preciso fazer cirurgia' },
      ],
      name: 'Maria',
      birthDate: '1990-03-15',
    ...
    const mockResponse = 'Parabéns! Você foi qualificada. Vamos agendar uma avaliação gratuita?';
    jest.spyOn(mockLLM, 'invoke').mockResolvedValue(mockResponse);

    const result = await generateResponseNode(state);

    expect(result.response).toBe('Parabéns! Você foi qualificada. Vamos agendar uma avaliação gratuita?');
    const callArgs = (mockLLM.invoke as jest.Mock).mock.calls[0][0];
    expect(callArgs).toContain('qualificado');
    expect(callArgs).toContain('Maria');
    expect(callArgs).toContain('Preciso fazer cirurgia e o médico exigiu perder peso');
  });
  ...
  it('should generate response for rejected step', async () => {
    ...
    const mockResponse = 'Obrigada pelo interesse. Infelizmente não podemos ajudá-la no momento.';
    jest.spyOn(mockLLM, 'invoke').mockResolvedValue(mockResponse);

    const result = await generateResponseNode(state);

    expect(result.response).toBe('Obrigada pelo interesse. Infelizmente não podemos ajudá-la no momento.');
    const callArgs = (mockLLM.invoke as jest.Mock).mock.calls[0][0];
  ...
  it('should return error message on API error', async () => {
    ...
    expect(result.response).toBe('Desculpe, ocorreu um erro. Por favor, tente novamente.');
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error generating response:', expect.any(Error));
