...
describe("analyzeMessageNode", () => {
  ...
  it("should extract name from message", async () => {
    const state: ConversationState = {
      ...
      messages: [{ role: "user", content: "Meu nome é Maria Silva" }],
      name: null,
      birthDate: null,
    ...
    const mockResponse = {
      content:
        '{"name": "Maria Silva", "birthDate": null, "weightLossReason": null}',
    };
  ...
  it("should extract birth date from message", async () => {
    const state: ConversationState = {
      phoneNumber: "5511999999999",
      messages: [{ role: "user", content: "Nasci em 15/03/1990" }],
      name: "Maria",
      birthDate: null,
      weightLossReason: null,
      qualified: null,
    ...
    const mockResponse = {
      content:
        '{"name": null, "birthDate": "1990-03-15", "weightLossReason": null}',
    };
    jest.spyOn(mockLLM, "invoke").mockResolvedValue(mockResponse as any);
  ...
  it("should extract weight loss reason from message", async () => {
    const state: ConversationState = {
      ...
      messages: [
        {
          role: "user",
          content: "Preciso fazer cirurgia e o médico exigiu perder peso",
        },
      ],
      name: "Maria",
    ...
    const mockResponse = {
      content:
        '{"name": null, "birthDate": null, "weightLossReason": "Preciso fazer cirurgia e o médico exigiu perder peso"}',
    };
    jest.spyOn(mockLLM, "invoke").mockResolvedValue(mockResponse as any);

    const result = await analyzeMessageNode(state);

    expect(result.weightLossReason).toBe(
  ...
  it("should preserve existing values when not extracted", async () => {
    ...
    const mockResponse = {
      content: '{"name": null, "birthDate": null, "weightLossReason": null}',
    };
    jest.spyOn(mockLLM, "invoke").mockResolvedValue(mockResponse as any);
  ...
  it("should parse JSON from response string", async () => {
    ...
    const mockResponse =
      'Here is the JSON: {"name": "João", "birthDate": null, "weightLossReason": null}';
    jest.spyOn(mockLLM, "invoke").mockResolvedValue(mockResponse as any);
  ...
  it("should handle string response directly", async () => {
    const state: ConversationState = {
      phoneNumber: "5511999999999",
      messages: [{ role: "user", content: "Meu nome é Pedro" }],
      name: null,
      birthDate: null,
      weightLossReason: null,
      qualified: null,
    ...
    const mockResponse =
      '{"name": "Pedro", "birthDate": null, "weightLossReason": null}';
  ...
  it("should handle response with content property", async () => {
    ...
    const mockResponse = {
      content: '{"name": "Ana", "birthDate": null, "weightLossReason": null}',
    };
    jest.spyOn(mockLLM, "invoke").mockResolvedValue(mockResponse as any);
  ...
  it("should include conversation context in prompt", async () => {
    ...
    const mockResponse = {
      content:
        '{"name": "Carlos", "birthDate": null, "weightLossReason": null}',
    };
    jest.spyOn(mockLLM, "invoke").mockResolvedValue(mockResponse as any);
